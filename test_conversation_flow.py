...
─────────────────────────
End-to-end tests for the 6 conversational command improvements.
...
check("Route returns clarification_needed", route_result["status"] == "clarification_needed")
check("Options contain Rahul Gupta", "Rahul Gupta" in route_result.get("options", []))

state = get_state(USER_ID)
check("State stored with pending_intent", state is not None and state.get("pending_intent") == "Add_Credit")
check("State stored with pending_amount = 200", state is not None and state.get("pending_amount") == 200)

# Simulate: user replies "Rahul Gupta"
reply_result = handle_clarification_reply("Rahul Gupta", USER_ID, state)
check("Clarification reply succeeds", reply_result.get("result", {}).get("status") == "success")
check("State cleared after execution", get_state(USER_ID) is None)


# ─── Test 3: Invoice creation flow ───────────────────────────────────────────
...
check("Invoice start asks for item", "item" in (invoice_start.get("message", "").lower()))
...
check("Step 1 asks for price", "price" in (step1.get("result", {}).get("message", "").lower()))

state = get_state(USER_ID)

# Step 2: User says "50" (price)
step2 = handle_invoice_step("50", USER_ID, state)
check("Step 2 asks for quantity", "quantity" in (step2.get("result", {}).get("message", "").lower()))

state = get_state(USER_ID)

# Step 3: User says "10" (quantity)
step3 = handle_invoice_step("10", USER_ID, state)
check("Step 3 creates invoice successfully", step3.get("result", {}).get("status") == "success")
check("Invoice message contains total", "500" in str(step3.get("result", {}).get("message", "")))
...
normalized = normalize_input("राहुल गुप्ता ने 500 दे दिए")
check("Devanagari normalized to roman", "rahul" in normalized.lower())
check("Numbers preserved", "500" in normalized)

normalized2 = normalize_input("Rahul ne 200 diye")
check("Already-roman text passes through", "rahul" in normalized2.lower())
